...
class Fourier_basis_Adaptative(FunctionBasis):
    def __init__(self, cfg, *args, **kwargs) -> None:
            ...
            self.ks = torch.nn.Parameter(torch.arange(1, self.N+1, 1, device=self.device).float(), True)
    
    def to(self, device):
        self.device=device
        self.ks = self.ks.to(self.device).detach().requires_grad_(True)
    ...
    def get_basis_derivativex(self, x):
        ...
        ins = torch.einsum('Nj, k -> Njk', x, idxs)
        # thetakf
        kfs1 = self.ff * idxs.unsqueeze(-1) 
        kfs2 = self.ff * idxs.unsqueeze(-1) 
        cs = torch.einsum('Njk, Nkc -> Njc', torch.sin(self.ff * ins), -kfs1) # j = x, n = channels, k = N chosen
        sn = torch.einsum('Njk, Nkc -> Njc', torch.cos(self.ff * ins), kfs2)
